...
В данном проекте реализована архитектура MVP (Model-View-Presenter). Взаимодействие между компонентами системы организовано через механизм событий — классы обмениваются данными и реагируют на изменения, подписываясь на соответствующие события.
...
- `gallery:changed` - обновление данных галереи (продуктов), перерисовка карточек на основе новых данных модели;
- `card:select` - выбор карточки, сохранение её данных в модели;
- `preview:changed` - обновление превью, отрисовка детальной карточки товара в модальном окне;
- `basket:changed` - изменение состава корзины, перерисовка элементов на основе данных модели;
- `basket:open` - открытие корзины;
- `modal:open` - открытие модального окна (блокировка главной страницы);
- `modal:close` - закрытие модального окна (разблокировка главной страницы);
- `order:open` - открытие формы заказа в модальном окне;
- `formErrors:change` - изменение ошибок валидации (активация/деактивация кнопки отправки);
- `order.payment:change` - выбор способа оплаты, сохранение в модель заказа;
- `order.address:change` - изменение адреса доставки, сохранение в модель заказа;
- `order:submit` - успешная валидация заказа, переход к форме контактов;
- `contacts.email:change` - изменение email, сохранение в модель заказа;
- `contacts.phone:change` - изменение телефона, сохранение в модель заказа;
- `contacts:submit` - успешная валидация контактов, отправка заказа и отображение подтверждения.